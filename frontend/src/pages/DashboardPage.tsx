  ...
  useEffect(() => {
    api.checkHealth().then((res) => {
      if (res.success && res.data) {
        ...
        setApiStatus(`API ${data.status} (v${data.version})`);
      } else {
        setApiStatus('API Standby');
      }
    });
  }, []);
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1 max-w-xl">
              Your AI Study Copilot is ready. Upload lecture notes, chat with grounded citations, or take active recall quizzes.
            </p>
          </div>
          ...
          <Card className="p-6">
            ...
            <div className="text-center py-12 px-4 rounded-xl border border-dashed border-slate-800 bg-slate-950/40">
              ...
              <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 mb-4">
                Upload your first lecture PDF or study guide to unlock grounded chat and quiz generation.
              </p>
              <Link to="/documents">
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <p className="text-xs text-slate-400 leading-relaxed mb-4">
              As you take quizzes, StudyPilot automatically tracks questions you miss and tags topics requiring revision.
            </p>
            <div className="p-4 rounded-xl bg-slate-950/50 border border-slate-800 text-center">
              <span className="text-xs text-slate-500">Take your first quiz to generate topic analysis</span>
            </div>
          </Card>
        </div>
      </div>
